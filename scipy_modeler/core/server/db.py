...
database.py (contains : 2 classes)
...
extend Cursor class (see cursor.py) to allow 
...
from scipy_modeler.core.server.cursor import (
        _con, Cursor
)
...
class DatabaseSchema():
    ...
    def __init__(self, schema_name: str, full_schema: Optional[bool] = False):
        ...
        assert schema_name in self._all, ValueError(
            'schema_name is not contained in database.')
    ...
    def all(self, new: list):
        ...
        assert isinstance(new, list), TypeError(
            "Must set '.all' with a list.")
        self._all = new
    ...
    def schema_name(self, new: str):
        ...
        assert isinstance(new, str), TypeError(
            "Must pass new schema as a string.")
    ...
    def schema_tree(self, new: Dict[str, str]):
        ...
        assert isinstance(new, dict), TypeError(
            "Must pass schema_tree as a dict.")
    ...
    def all_tables(self, new: List[str]): 
        ...
        assert isinstance(new, list), TypeError(
            "Must pass all_tables as a list.")
    ...
    def table_count(self, new: int):
        ...
        under the current schema. 
        """
        assert isinstance(new, int), TypeError(
            "Must pass table_count as an integer.")
    ...
    def add_schema(self, schema: str, position: Optional[int] = None):
        ...
        """
        assert isinstance(schema, str), TypeError(
            "Must pass new schema as a string.")
