...
master.py (contains : 1 function, 3 classes)
...
fit to the data and later used for inference. 
Learned models are persisted to disk as a binary
file which, when called on for inference on unseen
data, are read back into RAM as a Python object. 

Functions (1)
-------------
[1] _get_transformer_cols
...
from scipy_modeler.core.model.transform import Transformer
...
class SessionData():
    ...
    def struc(self, new: Dict[str, str]):
        ...
        assert isinstance(new, dict), \
            TypeError("Must pass a dictionary to struc "
                      "in order to set data types.")
        # dict keys and values must be strings
        assert all( isinstance(i, str) for i in list(new.keys()) ), \
            TypeError("Keys of dictionary passed must be strings.")
        assert all( isinstance(i, str) for i in list(new.values()) ), \
            TypeError("Values of dictionary passed must be strings.")
        self._struc = new
...
class SessionModel():
    ...
    def hyperparameters(self, new: Dict[str, Any]):
        ...
        assert isinstance(new, dict), \
            TypeError("Must pass a dictionary of hyperparameters.")
...
class SciPySession():
    ...
    def run_transformation_job(self, transform_params: Dict[str, Any]):
        ...
        """
        # SciKit Transformer object instantiated with the transformer params
        tf = Transformer(
                encoder=transform_params[
                    "categorical_config"]["encoder"],
                cat_imputer_strat=transform_params[
                    "categorical_config"]["cat_imputer_strat"],
                cat_imputer_val=transform_params[
                    "categorical_config"]["cat_imputer_val"],
                num_imputer_strat=transform_params[
                    "numerical_config"]["num_imputer_strat"],
                num_imputer_val=transform_params[
                    "numerical_config"]["num_imputer_val"],
                scaler=transform_params[
                    "numerical_config"]["scaler"],
                matrix_sparsity_threshold=transform_params[
                    "matrix_sparsity_threshold"], 
                ignore_cols=transform_params["ignore_cols"])
        ...
        self.pipeline.transformer = tf.build_transformer_pipeline(
