...
from scipy_modeler.core.query_locals import *
from scipy_modeler.util._settings import _cache
from scipy_modeler.util._decorators import send_logs
from scipy_modeler.core.table_met import Table, _all_types
from scipy_modeler.core.table_h import  _str_strip_all, _str_case_strip, \
    _flag_switch
...
class SciTable(Table):
    ...
    def save_struc(self) -> Dict[str,str]:
        ...
        # zip into dict with their respective dtypes as str
        # update final dict which each dtype dict
        for i in range(len(dtypes)):
            struc_order.append(self.list_by_dtype(dtypes[i])) 
            dtype_dicts.append(
                dict(zip(struc_order[i], [dtypes[i]*len(struc_order[i])])
                )
            )
    ...
    def expanded_view(self, nrow=100, ncol=100):
        """
    ...
    def info(self, memory='deep', complete=True):
        """
    ...
    @send_logs    
    def write_to_cache(self, file_type: str='csv', na_rep: Any='U/I'):
        """
        Write data to a csv or json file.
        """
        assert file_type in ('csv', 'json'), \
            ValueError("Must choose to write data to csv or json.")
        # replace existing
    ...
    def distinct(self, columns: List[str], checkpoint_before: bool=False, 
                    keep: str="first"):
        """
        ...
        if checkpoint_before:
            self.make_checkpoint()
            self.data.drop_duplicates(subset=columns, keep=keep, inplace=True)
            msg = (
    ...
    def random_permutation(self, size, seed=None):
        """
        ...
        Notes:
        -----
        If you want to get the same premutation you or a colleage got, 
        must pass the same integer to 'seed'
        """
        if seed:
            np.random.seed(seed)
        rows = sorted(np.random.permutation(len(self.data))[:size])
        return self.data.iloc[rows].copy()
            
    def clean_col_names(self, to_replace='', replace_with='', 
                         case=None, strip=False):
        """
    ...
    def rename_all_columns(self, new_names: List[str]):
        ...
        """
        assert len(self.data.columns) == len(new_names), \
            ValueError("Must provide the same number of new "
    ...
    def rename_columns(self, old_names: List[str], 
        ...
        rename_dict = {k:v for k,v in zip(self.data.columns, new_names)}
        self.data.rename(rename_dict, axis='columns', inplace=True)
        
    def add_prefix(self, prefix: str, include_dtype: Optional[str]=None,
        ...
        assert isinstance(columns, (str, list)), \
            TypeError("Must pass a string or list to 'ignore'.")
        assert all(col in self.data.columns for col in columns), \
            ValueError("One of more columns passed is not contained "
                      "in the current table.")
        assert not all(columns, include_dtype), \
            AssertionError("Cannot specify a data type to select "
        ...
        if columns == 'all':
            ...
            new_names = [f"{prefix}{col}" for col in self.data.columns]
        elif include_dtype:
    ...
    def add_suffix(self, suffix: str, include_dtype: Optional[str]=None,
                    columns: Optional[Union[str, List[str]]]='all'):
        """
        Add a prefix to all or some column names.
        ...
        ----------
        suffix : str
            A prefix to append to the ending of all or 
            specified column names.
        include_dtype : {'category', 'numeric', 'datetime', 'object'}, optional
            A datatype. Only include columns
            of this type when adding the specified prefix.
    ...
    def join(self, to, left_on, right_on, how='inner', 
             validate=None, indicator=False):
        """
        ...
        assert how in ('inner', 'outer', 'left', 'right', 'anti', 
                       'partial-outer'), \
            ValueError("Join type can only be one of the following: "
                       "'inner', 'outter', 'left', 'right', 'anti', "
                       "'partial-outter'.")   
        assert left_on in self.data.columns, \
            KeyError(f"Must join on a unique column of the current table. "
                     f"'{left_on}' does not exist in the current table.")
        assert right_on in to.data.columns, \
            KeyError(f"Must join to a unique column of the joining table. "
    ...
    def subset_columns(self, columns: Union[str, List[str]], 
        ...
        checkpoint_before : bool, optional
            ...
            Default = False.
        """
        assert action.lower() in ('exclude', 'include'), \
            ValueError("Must specify to either 'exclude' or 'include' "
                       "columns passed.")
        if checkpoint_before:
            self.make_checkpoint()
            msg = (
        ...
        if action.lower() == 'exclude':  
            ...
            if isinstance(columns, list):
                columns = [i for i in self.data.columns if i not in columns]
            else:
                columns = [i for i in self.data.columns if i != columns]
            self.data.drop(columns, axis=1, inplace=True)
    ...
    def select(self, to_select: Dict[str, Union[str,List[str]]], 
        ...
        for column in to_select.keys():
            assert column in self.data.columns, \
                KeyError(f"{column} not contained in table.")
            assert(is_categorical_dtype(self.data[column]) or 
                   is_string_dtype(self.data[column])), \
                   TypeError(
    ...
    def select_by_date(self, column: str, dates: Union[str, List[str]], 
                      action: Optional[str]='between', errors: Optional[str]='raise', 
                      checkpoint_before: Optional[bool]=False, 
        ...
        assert action.lower() in ('between', 'before', 'after'), \
            ValueError("action must be set to 'between', 'before', "
                       "or 'after'.")
        assert isinstance(dates, (str, list)), \
            TypeError("Must either pass one date as a string or multiple "
                      "dates in a list.")

        if (type(dates) == list) and (len(dates) > 1):
            assert action.lower() == 'between', \
                ValueError("When passing multiple dates, filter action may "
    ...
    def string_split_mutate(self, reference_column: str, split_on: str, 
        ...
        if keep_index.lower() == 'all':
            self.data[new_columns] = self.data[reference_column].str.split(
            ...
            self.data[new_columns] = self.data[reference_column].str.split(
                    split_on, expand=True)[keep_index]
    ...
    def strip_cols(self, columns: Optional[Union[List[str], str]] = 'all', 
        ...
        if columns.lower() == 'all':
            ...
            if type(columns) == list:
                assert all(i in self.data.columns for i in columns), \
                    KeyError("One or more columns specified are not contained "
                             "in the table.")
            else: 
            ...
            if case:
                assert case.lower() in ('lower', 'upper', 'title'), \
                    ValueError("case must be 'lower', 'upper', or 'title'.")
                    # see table_h
    ...
    def impute_missing_numeric(self, columns, how='median'):
        ...
        assert isinstance(columns, (str, list)), \
            TypeError("Can only pass a single column as a string or a list "
                      "of columns to fill.")
        assert how.lower() in ('median', 'mean'), \
            ValueError("Must choose to replace numeric columns with "
    ...
    def replace(self, to_replace: Union[str, int, float, list], 
                replace_with: Union[str, int, float, list], 
                columns: Optional[Union[str, List[str]]]='all', 
        ...
        Parameters
        ----------
        to_replace : str or int or float or list
            Values to replace in specified columns.
        replace_with : str ot in ot float or list
        ...
        """
        assert isinstance(columns, (str, list)), \
            TypeError("Must pass a column name as a string or a list "
                      "of columns.")

        if ( isinstance(to_replace, list) and isinstance(replace_with, list) ):
            assert len(to_replace) == len(replace_with), \
                AssertionError(
    ...
    def convert_to_numeric(self, columns: Union[str, List[str]],
        ...
        if isinstance(columns, list):
            assert all(i in self.data.columns for i in columns), \
                KeyError("One or more columns specified are not "
    ...
    def convert_to_datetime(self, columns: Union[str, List[str]],
        ...
        assert not all( (dt_format, infer_format) ), \
            ValueError("Cannot provide a datetime format while "
    ...
    def _cat_from_levels(self, columns, ordered=False) -> pd.Series:
        """
        Returns
        -------
    ...
    def convert_to_categorical(self, columns, ordinal=False):
        """
    ...
    def rename_all_categories(self, column: str, new_levels: List[str]):
        ...
        assert len(old_levels) == len(new_levels), \
            AssertionError(
    ...
    def bin_continous(self, reference_column: str, bins: List[int], 
                      levels: List[str], new_col_name: Optional[str]=None, 
    ...
    def set_to_flag(self, referrence_col: str, value: Any, 
                    check: str='equality', flag_value: Optional[Union[str, int]]=1, 
                    fill_value: Optional[Union[str, int]]=0, case: Optional[bool]=True, 
                    regex: Optional[bool]=False, drop_original: Optional[bool]=False):
        """
        ...
        assert isinstance(value, (str, int, float, list)), \
            TypeError("The value of interest used to generate a new flag "
                      "column must be a string, integer, float, or "
                      "list of values.")
            
        if regex and check.lower() != 'equality':
            raise AssertionError("Cannot check for anything but quality when "
                                 "using a regex value.")
            
        if ( isinstance(value, list) ) or ( value.lower() == "all" ):
            if isinstance(value, list):
                assert all(isinstance(i, (str, int, float)) for i in value), \
                TypeError("The reference value used to generate a new flag "
    ...
    def derive_new_flag(self, new_col_name: str, flag_when: str, 
    ...
    def make_numeric(self, columns: Union[str, List[str]], 
        ...
        if isinstance(columns, list) and (len(columns) > 1):
            for col in columns:
                if ( 
                        ( 
                            is_categorical_dtype(col) and 
                            max_levels is None
                        ) or (
                            len(col.cat.categories) > max_levels
                            )
                    ):
                    if inplace:
                        self.data[col] = self.data[col].cat.codes+1
                else:
                    self.data[
                        f"{col}_as_numeric"] = self.data[col].cat.codes+1
        else:
            if is_categorical_dtype(columns) and ((max_levels is None) or 
                                    len(columns.cat.categories) > max_levels):
                if inplace:
                    self.data[columns] = self.data[columns].cat.codes+1
            else:
                self.data[
                    f"{columns}_as_numeric"] = self.data[col].cat.codes+1
    ...
    def reindex_categorical_columns(self, label: str, move_to: Optional[str]='left', 
        ...
        """
        assert move_to.lower() in ('left', 'right'), \
            ValueError("move_to must be either 'left' or 'right'.")
        
        assert isinstance(label, str), \
            ValueError("Must pass a string representing "
                       "the label (target) column to 'label'.")
        assert label in self.data.columns, \
            KeyError("label Column passed  does not exist in data.")
...
########################## > Grand Children < ##########################
class AALEncodedTable(SciTable):
    """
